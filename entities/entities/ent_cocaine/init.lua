...
function ENT:Initialize()
	...
	self.GrowthTime = COCAINE_GROW_TIME

	self:GetPhysicsObject():Wake()
	self:GetPhysicsObject():SetDamping( 0, 1000 )

	--timer.Simple( 0, function() -- Next frame
	--	self.RefundTable = table.insert( SPAWNED_ITEMS, { Items = { 68, 15 }, Owner = self.maker } ) -- In case of a server crash it'll refund props
...
function ENT:Use( activator, caller )
	...
	if activator:Team() == TEAM_POLICE || activator:Team() == TEAM_SWAT || activator:Team() == TEAM_CHIEF || activator:Team() == TEAM_DETECTIVE || activator:Team() == TEAM_FBI then
		...
		activator:ChatPrint( "You have been rewarded $" .. GAMEMODE.CopReward_Arrest .. " for destroying cocaine." )
		self:Remove()
	...
	elseif self.SpawnTime + self.GrowthTime < CurTime() then
		...
		activator:GiveItem( 69, roll )

		self.Tapped = true

		if sroll ~= 0 then
			activator:GiveItem( 68, sroll )
		end
		...
		activator:Notify( "You've received " .. roll .. " cocaine grams. The pot has also been returned to your inventory." )
	elseif self.SpawnTime + self.GrowthTime * .5 < CurTime() then
		...
		activator:GiveItem( 15, 1 )
		activator:Notify( "You've received " .. roll .. " cocaine bags. The pot has also been returned to your inventory." )
	end
end
