...
function ENT:Initialize()
	...
	--self:SetModel(string.format("models/billiards/table_%ft.mdl", self.Size))
...
function ENT:Use(ply, caller)
	...
	if(self.PlayerData[1] != nil) then
		...
		umsg.End()
		ply:ChatPrint( "Request Sent!", "A request has been sent! Wait for the answer." )
...
function ENT:GetTableEnt(nm)
	if(!nm || nm == "CueBall") then
		return ents.GetByIndex( self.EntsData["CueBall"] or 0 )
	elseif(string.match(nm, "(%-Ball)$")) then
		local i = string.gsub(nm, "-Ball", "")
		i = tonumber(i)
		return self.Balls[i]
	else
		return ents.GetByIndex( self.EntsData[nm] or 0 )
	end
	return nil
end
