...
concommand.Add( "slotm_setbet", function( ply, cmd, args )
	if !Casino.SlotsSettingBet then
		...
		Derma_StringRequest( "Slot Machine", "Set the amount of money you would like to bet. (500 - 1000)", Casino.SlotsLocalBet, 
						function( strTextOut )
							local amount = tonumber( strTextOut ) or Casino.SlotsLocalBet
							Casino.SlotsLocalBet = math.Clamp( math.Round(amount), 500, Casino.SlotsMaxBet )
							Casino.SlotsSettingBet = false
						end,
						function( strTextOut )
							Casino.SlotsSettingBet = false
...
surface.CreateFont( "ScoreboardText", { font = "coolvetica", size = 18, weight = 500, antialias = true, additive = false } )
...
function ENT:DrawDisplay()
	...
	cam.Start3D2D( pos, ang, scale )	
		...
		draw.SimpleText( "Jackpot: $" .. util.FormatNumber( self:GetJackpot() ), "ScoreboardText", 10, 9, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
		draw.SimpleText( "Bet Amount: $" .. util.FormatNumber( Casino.SlotsLocalBet ), "ScoreboardText", 10, 80, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
		
		--old coords for the text, bet amount didnt dispaly
		--draw.SimpleText( "Jackpot: $" .. util.FormatNumber( self:GetJackpot() ), "ScoreboardText", 10, 25, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
		--draw.SimpleText( "Bet Amount: $" .. util.FormatNumber( Casino.SlotsLocalBet ), "ScoreboardText", 10, 98, Color(255,255,255,255), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP )
...
surface.CreateFont( "Buttons", { font = "coolvetica", size = 22, weight = 200, antialias = true, additive = false } )
