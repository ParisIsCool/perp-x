...
function EFFECT:Init( Data )
	...
	if not IsValid( self.Barfer ) or not self.Barfer:IsPlayer() then return false end
...
function EFFECT:Think()
	if not IsValid( self.Barfer ) or not self.Barfer:IsPlayer() or CurTime() > self.DieTime then self.Emitter:Finish() return false end
	...
	Particle:SetColor( Color( 128, 80, 0 ) )
