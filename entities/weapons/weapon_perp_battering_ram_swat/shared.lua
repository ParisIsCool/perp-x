...
function SWEP:PrimaryAttack()
	...
	if IsValid( EyeTrace.Entity ) or not EyeTrace.Entity:IsDoor() then return false end
...
function SWEP:SecondaryAttack()
	...
	local EyeTrace = self.Owner:GetEyeTrace()
	if not IsValid( EyeTrace.Entity ) or not EyeTrace.Entity:IsVehicle() then return false end
	
	local Distance = self.Owner:EyePos():Distance(EyeTrace.HitPos)
	
	if Distance > 75 then return false end
	
	if not IsValid( EyeTrace.Entity:GetDriver() ) or not EyeTrace.Entity:GetDriver():IsPlayer() then return false end
	
	if EyeTrace.Entity:GetVelocity():Length() < 25 then
