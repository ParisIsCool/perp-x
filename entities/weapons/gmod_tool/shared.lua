...
local ct = ""
local points = {}
AddGear( "Clear Clipboard", "TOOL: Clear Clipboard.", false,
...
AddGear( "Copy Job Spawnpoint", "TOOL: Left click to ADD current position to clipboard.", false,
	function( Player, Trace ) end, true, function(ply)
		...
		a.p = math.Round(a.p / 45) * 45
		a.y = math.Round(a.y / 45) * 45
		a.r = math.Round(a.r / 45) * 45
		ct = ct .. Format("\t{Vector(%i,%i,%i), Angle(%i,%i,%i)},\n",p.x,p.y,p.z,a.p,a.y,a.r)
		SetClipboardText(ct)
	end
)

AddGear( "TEMP: Copy Lookat Tool", "TOOL: Left click to ADD current hitpos to clipboard.", false,
	function( Player, Trace ) end, true, function(ply)
...
AddGear( "Get Vehicle Plates", "TOOL: Left click to ADD current hitpos to clipboard.", false,
	function( Player, Trace ) end, true, function(Player, Trace)
		if IsValid( Trace.Entity ) and Trace.Entity:IsVehicle() then
			...
			local vname = Trace.Entity:GetNWString( "veh_name" )
			local plates = LPlates.GetVehiclePlates( vname )

			local str ="RegisterVehiclePlates( \"" .. vname .. "\",\n\t{\n\t\tpos = Vector( " .. Pos.x .. "," .. Pos.y .. "," .. Pos.z .. " ),\n\t\tang = Angle( 0, 0, 90 ),\n\t\tscale = 0.035\n\t}\n)"
...
AddGear( "[SM] Set Bank Ready [0]", "Sets bank cash to $0 making it ready to rob.", false,
	function( Player, Trace )
		if not Player:IsServerManager() then
			return Player:Notify( "This gear requires Server Manager status." )
		end

		Player:Notify( "Bank set!" )
		Player:ConCommand("perp_setbanknotready")
		
	end
)

AddGear( "[SM] Activate National Guard", "Activates National Guard Status.", false,
	function( Player, Trace )
		if not Player:IsServerManager() then
			return Player:Notify( "This gear requires Server Manager status." )
		end

		Player:Notify( "National Guard Activated!" )
		Player:ConCommand("perp_activate_ng")
		
	end
)

AddGear( "[SM] Deactivate National Guard", "Deactivates the National Guard Status.", false,
	function( Player, Trace )
		if not Player:IsServerManager() then
			return Player:Notify( "This gear requires Server Manager status." )
		end

		Player:Notify( "National Guard Deactivated!" )
		Player:ConCommand("perp_deactivate_ng")
		
	end
)

AddGear( "[SM] Set Bank Ready [7200]", "Sets bank cash to $7200 making it ready to rob.", false,
	function( Player, Trace )
		if not Player:IsServerManager() then
			return Player:Notify( "This gear requires Server Manager status." )
		end
		...
		Player:Notify( "Bank set!" )
		Player:ConCommand("perp_setbankready")
...
AddGear( "[SM] Event Alert", "Left click to send event alert (Needs to be edited live).", false,
	function ( Player, Trace )
		if not Player:IsServerManager() then
			return Player:Notify( "This gear requires Server Manager status." )
		end
		...
		for k, v in pairs(player.GetAll()) do
			v:Notify( "EVENT ALERT" )
			v:Notify( "Please set me up in shared.lua file." )
		end
...
AddGear( "[O] Disable Brake", "Disabled hand break on car.", false,
	function( Player, Trace )
		if not Player:IsOwner() then
			return Player:Notify( "This gear requires Owner status." )
		end

		for _, v in pairs( Trace.Entity ) do
				v:Fire( "HandBrakeOff" )
		end
	end
)
...
AddGear( "Random Cars", "Left click to create random cars.", false,
	function( Player, Trace )
		...
		local randCar = VEHICLE_DATABASE["BLAZE"]

		GAMEMODE.SpawnVehicle( Player, randCar.ID, { RGBColour = Color( math.random( 0, 255), math.random( 0, 255), math.random( 0, 255), 255 ), PaintID = 1, HeadLights = 1 }, { Trace.HitPos, Player:GetAngles() }, true )
