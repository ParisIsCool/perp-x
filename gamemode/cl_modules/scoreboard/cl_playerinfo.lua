...
PLAYERINFO.InfoValues = {
	...
	[5] = { "Job",
		function( ply )
			...
			local class = JOB_DATABASE[ ply:Team() ].Name
			if class then
				return class
			end
...
PLAYERINFO.AdminButtons = {
	[1] = { "Kick",
		function( ply )
			PromptStringRequest( "Custom Reason...",
				"Why do you want to kick " .. ply:Nick() .. "?",
				"",
				function( strTextOut )
					RunConsoleCommand("asoc_kick",ply:SteamID(), strTextOut)
				end
			)
		end
	},
	...
	[2] = { "Ban",
		function( ply )
			...
			PromptStringRequest( "Custom Reason...",
				"Why do you want to ban " .. ply:Nick() .. "?",
				"",
				function( strTextOut )
					local time = GAMEMODE.AdminPunishment_Time:GetInt()
					RunConsoleCommand("asoc_ban",ply:SteamID(), strTextOut, time)
				end
			)
		end
	},
	...
	[4] = { "Blacklist",
		function( ply )
			...
			Menu:AddOption( "Help", function()
				ShowGetBanTime( "Help Blacklisting " .. ply:Nick(), "Blacklist Reason", "Help Blacklist Player", function ( t )
					if IsValid( ply ) and ply:IsPlayer() then
						Blacklist( ply, "help", t )
					end
				end )
			end )
	...
	[5] = { "Unblacklist",
		function( ply )
			...
			for k, v in pairs( Blacklists ) do
				...
				elseif k == "help" and ply:HasBlacklist( "help" ) then
					Menu:AddOption( "Help (" .. v.Reason .. ")", function() Unblacklist( ply, "help" ) end )
				elseif k == "job" then
...
vgui.Register( "ScoreboardPlayerInfo", PLAYERINFO, "DPanel" )
