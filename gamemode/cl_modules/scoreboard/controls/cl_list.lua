...
function PANEL:Init()
	...
	self.VBar.btnGrip.Paint = function( panel, w, h )
		...
		if ( panel:GetDisabled() ) then
			draw.RoundedBox( 0, 2, 1, w, h, self.Scrollbar.DisabledColor )
			return
		end
		
		if ( panel.Depressed ) then
			draw.RoundedBox( 0, 2, 1, w, h, self.Scrollbar.DepressedColor )
			return
		end
		
		if ( panel.Hovered ) then
			draw.RoundedBox( 0, 2, 1, w, h, self.Scrollbar.HoverColor )
			return
		end
		
		draw.RoundedBox( 0, 2, 1, w, h, self.Scrollbar.BackgroundColor )
...
derma.DefineControl( "DPanelListPE", "", PANEL, "DPanelList" )