...
surface.CreateFont( "MixturesInfoFont", { font = "Tahoma", size = ScreenScale( 6 ), weight = 1000, antialias = true, additive = false } )
...
function PANEL:PerformLayout ( )
	...
	if self.requiredSkillsText then
...
function PANEL:Think()
	...
	for _, req in pairs(self.Mixture.Requires) do
		if(LocalPlayer():GetSkillLevel(req[1]) < req[2]) then
			Has = false
		end
	end
...
function PANEL:SetMixture ( mID )
	...
	if (self.Mixture.RequiresSawHorse) then
		self.requiresText = self.requiresText .. "Saw Horse, "
	end
