...
function PANEL:Init ( )
	...
	self.removeBuddies = vgui.Create("perp_button", panelList)
	self.removeBuddies:SetButtonText("Remove Buddy")
	self.panelList:AddItem(self.removeBuddies)
	
	self.addBuddies = vgui.Create("perp_button", panelList)
	self.addBuddies:SetButtonText("Add Buddy")
...
function PANEL:SaveFile()
	...
	file.Write( "pe_buddies.txt", util.TableToJSON( GAMEMODE.Buddies ) )
end
...
function PANEL:Think()
	...
	elseif self.wasSaving then
		self.wasSaving = nil
		file.Delete("pe_buddies.txt")
	end
end

vgui.Register("perp2_buddies", PANEL, "perp_frame")