...
NPC.PosAngle ['rp_southside'] = { 
	{pos=Vector(9375,1535,-103),ang=Angle(2,179,0)},
}
...
function NPC.OnTalk()
	if LocalPlayer():Team() == TEAM_FIREMAN then
		...
		if LocalPlayer():HasVehicleOut() then
			GAMEMODE.DialogPanel:AddDialog("I want to put my firetruck up.", function() 
	...
	elseif LocalPlayer():Team() == TEAM_CITIZEN then
		GAMEMODE.DialogPanel:SetDialog("Hello. Are you here to report a fire?")

		GAMEMODE.DialogPanel:AddDialog("I'm actually interested in becoming a fireman.", NPC.Hire)
		GAMEMODE.DialogPanel:AddDialog("No. Just having a look around.", LEAVE_DIALOG)
	else
...
function NPC.Hire()
	...
	elseif LocalPlayer():HasBlacklist( "job", TEAM_FIREMAN ) then
	...
	elseif (LocalPlayer():GetTimePlayed() < JOB_DATABASE[TEAM_FIREMAN].RequiredTime * 60 * 60 and !LocalPlayer():IsVIP()) then
		GAMEMODE.DialogPanel:SetDialog("Sorry, but it appears that you are not qualified for this position.\n\n(You need at least " .. JOB_DATABASE[TEAM_FIREMAN].RequiredTime .. " hours of play time or VIP to be a fireman.)")
	...
	elseif LocalPlayer():HasVehicleOut() then
		...
		GAMEMODE:RequestJobJoin(TEAM_FIREMAN)
...
function NPC.RequestCar(vehicle)
	...
	for _, v in pairs( player.GetHumans() ) do
		if v:GetGNWVar( "Vehicle" ) then
			...
			if VehicleTable and VehicleTable.RequiredClass == TEAM_FIREMAN then
